...
def normalize_common_full_config(config: dict) -> None:
    ...
    if _positive_number(config.get("max_train_steps")):
        # Both SDXL and Flux recalculate max_train_steps whenever epochs is set.
...
def _validate_cacheability(config: dict, label: str) -> None:
    ...
    te_cache = _as_bool(config.get("cache_text_encoder_outputs")) or _as_bool(
    ...
    if te_cache:
        ...
        if incompatible:
            raise ValueError(
                f"{label}: text-encoder output cache 不能与 caption dropout / shuffle / token warmup / tag dropout 同时启用。"
            )
...
def normalize_validate_sdxl_full(config: dict) -> None:
    ...
    if _as_bool(config.get("diffusers_xformers")) and (
        _as_bool(config.get("xformers")) or _as_bool(config.get("sdpa"))
    ):
