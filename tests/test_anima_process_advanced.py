...
from mikazuki.process import (
    _normalize_anima_save_schedule,
    _validate_anima_finetune_advanced_combinations,
    _validate_effective_text_encoder_cache,
)
...
class AnimaProcessAdvancedTests(unittest.TestCase):
    ...
    def test_masked_loss_requires_conditioning_source(self):
        with self.assertRaisesRegex(ValueError, "masked_loss"):
            _validate_anima_finetune_advanced_combinations({"masked_loss": True})
